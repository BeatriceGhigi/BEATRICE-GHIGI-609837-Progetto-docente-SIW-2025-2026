...
public class TorneoController {
	...
	public TorneoController(TorneoService torneoService, SquadraService squadraService) { // costruttore
	...
	public String show(@PathVariable("id") Long id, Model model) {
		...
		model.addAttribute("torneo", torneo.get()); // metto a disposizione del componente che genera l'html, il componente
		return "tornei/show";	 // che mette a disposizione l'html puù generare quest'oggetto
										// attraverso quetso nome qua 
	}

	@GetMapping("/tornei") /* questo metodo deve prendere tutti tornei e passarli al template */
	...
	public String form(Model model) {
	 ...
	 @PostMapping("/admin/tornei")
	    public String save(@Valid @ModelAttribute("torneo") Torneo torneo,
	        ...
	        // Ricostruisce la lista attori dagli hidden input
	        List<Squadra> squadre = new ArrayList<>();
	        if (squadraIds != null)
	            for (Long id : squadraIds) {
	 ...
	 @GetMapping("/admin/tornei/{id}/edit")
	    public String editForm(@PathVariable Long id, Model model) {
	        ...
	        Torneo optional = torneo.get();
	        if (optional.getSquadre() == null) optional.setSquadre(new ArrayList<>());
	        model.addAttribute("torneo", optional);
	        model.addAttribute("squadre", squadraService.findAll());
	        return "admin/tornei/form";
	    }
